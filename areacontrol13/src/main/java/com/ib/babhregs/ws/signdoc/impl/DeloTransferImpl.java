package com.ib.babhregs.ws.signdoc.impl;
...
import com.ib.babhregs.ws.signdoc.DeloTransfer;
import com.ib.babhregs.ws.signdoc.WSFault;
...
@WebService(serviceName = "DeloTransferService", endpointInterface = "com.ib.babhregs.ws.signdoc.DeloTransfer", targetNamespace = "http://wstransfer.delo.indexbg.com/")
...
//			
//				fake.generatеFakeNotif(BabhConstants.CODE_FAKE_NOTIF_RELOAD_FILES_SIGN);
//			
...
//		}
	...
	public java.lang.String uploadFile(byte[] fileContent, java.lang.String fileName) throws WSFault {
		...
		if (fileName == null || fileName.trim().length() < 44) {
			...
			//DocuWrk.0.4509.76eadff3ab9b4d88942da7634b8cf663.3344.docx
		...
		try {
			...
			Integer reload = BabhConstants.CODE_FAKE_NOTIF_RELOAD_FILES_SIGN;
			if (docId > 0) {
				Doc doc = ddao.findById(docId);
				if (doc != null) {
					if (!doc.getDocType().equals(BabhConstants.CODE_ZNACHENIE_DOC_TYPE_WRK)) {
						...
						sqlQuery.setParameter("DANE", BabhConstants.CODE_ZNACHENIE_NE);
						sqlQuery.setParameter("FILEID", file.getId());
						sqlQuery.setParameter("DOCCODE", BabhConstants.CODE_ZNACHENIE_JOURNAL_DOC);
						...
						newFile.setOfficial(BabhConstants.CODE_ZNACHENIE_DA);
						...
						fdao.saveFileObject(newFile, docId, BabhConstants.CODE_ZNACHENIE_JOURNAL_DOC);
			...
			sqlQuery.setParameter("TIPO", BabhConstants.CODE_ZNACHENIE_JOURNAL_FILE);
			...
			if(taskId > 0 && signed == 1 && countSign < countSignNew) {
				...
				reload = BabhConstants.CODE_FAKE_NOTIF_RELOAD_FILES_SIGN_TASK;
			}
