...
import static com.ib.babhregs.system.BabhConstants.CODE_CLASSIF_BOSS_POSITIONS;
import static com.ib.babhregs.system.BabhConstants.CODE_CLASSIF_REGISTRI;
import static com.ib.babhregs.system.BabhConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_USER_LOCKED;
import static com.ib.babhregs.system.BabhConstants.CODE_ZNACHENIE_NOTIF_ROLIA_CONTR;
 ...
 */
@ApplicationScoped
public class BabhIdentityStore extends IBIdentityStore {

	private static final Logger LOGGER = LoggerFactory.getLogger(BabhIdentityStore.class);

	@Inject
	private ServletContext servletContext;

	/**Към този момент работи 1:! както е DBCredential
	 * @param credential
	 * @return
	 * @throws AuthenticationException
	 */
	protected Optional<BaseUserData> findUserREST(RestCredential credential) throws AuthenticationException {
		LOGGER.debug("findUserREST: username={},password={}",credential.getCaller(),credential.getPasswordAsString());
		return findUserDB(new DBCredential(credential.getCaller(),credential.getPasswordAsString()));
	}

	/**Този метод НЕ рови в базата.
	 * Всички данни се вземат от токена
	 * @param credential тук е токен-а, от който ако е валиден да извлечем информация, да конструираме принципал-а
	 * @return
	 * @throws AuthenticationException
	 */
	@Override
	protected Optional<BaseUserData> findUserToken(TokenCredential credential) throws AuthenticationException {
		LOGGER.debug("findUserToken: token={}",credential.getToken());
		SystemData systemData = (SystemData) this.servletContext.getAttribute("systemData");
		String tokenSecretKey=null;
        try {
            tokenSecretKey=systemData.getSettingsValue("tokenSecretKey");
        } catch (DbErrorException e) {
			LOGGER.info("Thee is not \"tokenSecretKey\" property in SYSTEM_OPTIONS. Will be used default!!!");
        }

		UserData userData = null;//
		try {
			LOGGER.debug("Going to use tokenSecretKey:{}",tokenSecretKey);
			LOGGER.debug("Going to parse token:{}",credential.getToken());
			Jws<Claims> claimsJws = new JWTUtil(tokenSecretKey).decodeJWT(credential.getToken());
			Claims payload = claimsJws.getPayload();
			Integer userId = Integer.valueOf(payload.get("userId").toString());
			String subject = payload.getSubject();
			String names = payload.get("names").toString();
			LOGGER.debug("Parsed data from token. userId:{}, subject:{}, names:{}",userId,subject,names);
			userData=new UserData(userId, subject, names);
		} catch (Exception e) {
	...
	protected Optional<BaseUserData> findUserDB(DBCredential credential) throws AuthenticationException {
		LOGGER.debug("findUserDB: user:{}, pass:{}",credential.getCaller(),credential.getPassword());
	...
	protected Optional<BaseUserData> findUserLDAP(LDAPCredential credential) throws AuthenticationException {
		...
		try {
			...
			String defaultDomain = systemData.getSettingsValue(BabhConstants.DEFAULT_LDAP_DOMAIN);
	...
	public UserData createUserData(SystemData sd, AdmUser user) throws DbErrorException {
		...
		if (row != null) {
			...
			userData.setRegistratura((Integer) row.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA]);
			userData.setZveno(row.getCodeParent()); // CODE_PARENT се явява звеното на човека

			Integer position = (Integer) row.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_POSITION];
			...
			if (position != null && userData.getZveno() != null) {
				...
				if (boss) { // трябва да се изтеглят и звената до които има достъп !!!
					...
					Map<Integer, Object> specifics = Collections.singletonMap(BabhClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE, CODE_ZNACHENIE_REF_TYPE_ZVENO);
		...
		if (userData.getRegistratura() != null) { // пускаме само ако текущия работи в регистратура
			...
			Map<Integer, Boolean> userMailboxes = user.getAccessValues().get(BabhConstants.CODE_CLASSIF_MAILBOXES);
			if (userMailboxes != null && !userMailboxes.isEmpty()) { // само ако има зададени

				// пощенските кутии за регистратурата
				List<SystemClassif> items = sd.queryClassification(BabhConstants.CODE_CLASSIF_MAILBOXES, null, today, userData.getCurrentLang()
					, Collections.singletonMap(BabhClassifAdapter.MAILBOXES_INDEX_REGISTRATURA, userData.getRegistratura()));
		...
		Map<Integer, Boolean> kontrol = user.getAccessValues().get(BabhConstants.CODE_CLASSIF_OBLAST_KONTROL);
		userData.setLimitedAccess(kontrol == null || kontrol.isEmpty());
