 ...
 * @author belev
 */
public class BabhSystemDataSynchronizer extends SystemDataSynchronizer {

	private static final Logger LOGGER = LoggerFactory.getLogger(BabhSystemDataSynchronizer.class);

	/**
	 * @param systemData
	 * @param seconds
	 */
	BabhSystemDataSynchronizer(BaseSystemData systemData, Long seconds) {
...
//		ADM_REFERENTS - Constants.CODE_CLASSIF_ADMIN_STR
		...
		if (!list.isEmpty()) {
			codes.add(Constants.CODE_CLASSIF_ADMIN_STR);
			codes.add(SysConstants.CODE_CLASSIF_USERS);
			codes.add(BabhConstants.CODE_CLASSIF_ADMIN_STR_REPORTS);
		}
...
//		REGISTRATURA_GROUPS
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			...
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(BabhConstants.CODE_CLASSIF_GROUP_EMPL);
		}
...
//		REGISTRI
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			...
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(BabhConstants.CODE_CLASSIF_REGISTRI);
			codes.add(BabhConstants.CODE_CLASSIF_REGISTRI_SORTED);
			codes.add(BabhConstants.CODE_CLASSIF_REGISTRATURI); // заради списъка от регистри като специфика
		} else {
//		REGISTRATURI
			list = JPA.getUtil().getEntityManager().createNativeQuery( //
				...
				.setParameter(h2DateParam, h2Date).getResultList();
			if (!list.isEmpty()) {
				codes.add(BabhConstants.CODE_CLASSIF_REGISTRATURI);
			}
		}

//		DOC_VID_SETTINGS - BabhConstants.CODE_CLASSIF_DOC_VID_SETTINGS
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			"select max (a.ID) as syncme from DOC_VID_SETTINGS a having max (a.DATE_REG) > :h2Date or max (a.DATE_LAST_MOD) > :h2Date") //
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(BabhConstants.CODE_CLASSIF_DOC_VID_SETTINGS);
		}
//		DOC_VID_SETTINGS - BabhConstants.CODE_CLASSIF_DOC_VID_SETTINGS - през файловете заради шаблоните
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			...
			.setParameter("objCode", BabhConstants.CODE_ZNACHENIE_JOURNAL_DOC_VID_SETT).setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(BabhConstants.CODE_CLASSIF_DOC_VID_SETTINGS);
		}

//		REGISTRATURA_MAILBOXES - BabhConstants.CODE_CLASSIF_MAILBOXES
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			...
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(BabhConstants.CODE_CLASSIF_MAILBOXES);
		}

//		PROC_DEF - BabhConstants.CODE_CLASSIF_PROCEDURI
		list = JPA.getUtil().getEntityManager().createNativeQuery( //
			"select max (a.DEF_ID) as syncme from PROC_DEF a having max (a.DATE_REG) > :h2Date or max (a.DATE_LAST_MOD) > :h2Date") //
			.setParameter(h2DateParam, h2Date).getResultList();
		if (!list.isEmpty()) {
			codes.add(BabhConstants.CODE_CLASSIF_PROCEDURI);
			codes.add(BabhConstants.CODE_CLASSIF_DOC_VID_SETTINGS);
		}
