...
public class SelectReferents extends UINamingContainer {
	...
	public void initRefComp() {	
		if( (Referent) getAttributes().get("refObj") != null ) {
			Referent tmpRef=(Referent) getAttributes().get("refObj");
			setSearchWord(tmpRef.getRefName());
		}else {
			if (getAttributes().get("selectedCode")!=null) {
				try {
					
					setSearchWord(getSystemData().decodeItem(BabhConstants.CODE_CLASSIF_REFERENTS, (Integer)getAttributes().get("selectedCode"), getUserData().getCurrentLang(), getDateClassif()));
	...
	public void search() throws DbErrorException, InvalidParameterException {
		...
		if (typeSearch.equals("EGN")) {
			if (getSearchWord() != null && getSearchWord().length() > 0) {
				...
				if (ValidationUtils.isValidEGN(getSearchWord())) {
					goSearch=true;
				}else {
			...
			if (typeSearch.equals("EIK")) {
				if (getSearchWord() != null && getSearchWord().length() > 0) {
					if (ValidationUtils.isValidBULSTAT(getSearchWord())) {
						goSearch=true;
					}else {
		...
		if (goSearch) {
				...
				try {
					List<SystemClassif> classifList = getSystemData().getSysClassification(BabhConstants.CODE_CLASSIF_REFERENTS, getDateClassif(),  getUserData().getCurrentLang()); 
