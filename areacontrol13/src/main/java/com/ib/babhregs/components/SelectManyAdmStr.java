...
//	}
	...
	public void loadRoot()  {
		...
		if (getTreeType().equals(ADM_LIST)){
			...
			codeClassif = BabhConstants.CODE_CLASSIF_GROUP_EMPL;
			bb = true;
		}			
			...
			if (getRoot() == null) {
				...
				try {
					...
					if(filtered) {
						...
						readOnlyCodes = null;
						expanded = true;
						Map<Integer, Object> specRegistratura = Collections.singletonMap(BabhClassifAdapter.REG_GROUP_INDEX_REGISTRATURA, ((UserData) getUserData()).getRegistratura());
	...
	private void onNodeSelectAdmStr(NodeSelectEvent event) {
		...
		if (bb) {
			...
			boolean zz = true;
			if( getZvena() != null && !getIsSelectNode() && getZvena().contains(selectedItem)) {
				// това е проверка да забрани избора на звено, ако дървото е било изградено след търсене!
				zz = false;			
				String clientId =  this.getClientId(FacesContext.getCurrentInstance());
				JSFUtils.addMessage(clientId+":treeM",FacesMessage.SEVERITY_ERROR, "Изборът е забранен!");
			} 
	...
	private void onNodeSelectAdmGr(NodeSelectEvent event)  {
		...
		String emplCodes;
		try {
			emplCodes = (String) getSystemData().getItemSpecific(BabhConstants.CODE_CLASSIF_GROUP_EMPL, selectedItem.getCode(), SysConstants.CODE_DEFAULT_LANG, getDateClassif(), BabhClassifAdapter.REG_GROUP_INDEX_MEMBERS);
	...
	public void search() throws DbErrorException {
		...
		try {
			...
			if(getTreeType().equals(ADM_GR)) {
				...
				if(!getIsSelectNode()&& getZvena() == null) {
					// необходимо ми е, ако при търсенето звеното остане на най-ниско ниво и тогава да мога да проверя, ако е забранено да се избира звено!
					Map<Integer, Object> specs = new HashMap<>();
			        specs.put(BabhClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE, BabhConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO);
			        List<SystemClassif> tmp = getSystemData().queryClassification(BabhConstants.CODE_CLASSIF_ADMIN_STR, null, dateClassif, getUserData().getCurrentLang(), specs);
					setZvena(tmp);
				}
 ...
 */
	...
	@SuppressWarnings("unchecked")
	public List<SystemClassif> getZvena() {
		return (List<SystemClassif>) getStateHelper().eval(PropertyKeys.ZVENA, null);
	}


	public void setZvena(List<SystemClassif> zvena) {
		getStateHelper().put(PropertyKeys.ZVENA, zvena);
	}
