...
public class PersonalMessages extends IndexUIbean implements Serializable {
	...
	@PostConstruct
	void initData() {
		// TODO скрито е показването на бройката и е излишно и да се пуска селект
//		try {
//
//			UserNotificationsDAO dao = new UserNotificationsDAO();
//
//			setBrNotif(dao.countUserNotifications(getUserData(UserData.class).getUserAccess(), BabhConstants.CODE_NOTIF_STATUS_NEPROCHETENA, getUserData(UserData.class).getRegistratura()));
//
//		} catch (Exception e) {
...
//		}
	...
	public void actionLoadMessages() {
		...
		try {
			...
			ArrayList<UserNotifications> messages = dao.findUserNotifications(getUserData(UserData.class).getUserAccess(),BabhConstants.CODE_NOTIF_STATUS_NEPROCHETENA, getUserData(UserData.class).getRegistratura());
	...
	public String actionGoto(UserNotifications uNotif) {
		...
		if(uNotif.getCodeObject() == BabhConstants.CODE_ZNACHENIE_JOURNAL_PROC_EXE) {
