package com.ib.babhregs.notifications;
...
import com.ib.babhregs.db.dao.UserNotificationsDAO;
	...
	public void processNotificationAsync(@ObservesAsync Notification nb) {
		...
		try {
			...
			if (nb.getNotificationCode() != null ) {
				...
				if (!nb.getSd().matchClassifItems(BabhConstants.CODE_CLASSIF_NOTIFF_EVENTS_ACTIVE, nb.getNotificationCode(), new Date())) {
					return;
				}
				...
				for (Integer referent : nb.getAdresati()) {
					...
					if (! nb.getSd().matchClassifItems(BabhConstants.CODE_CLASSIF_USERS, referent, new Date())) {
						//референтът не е потребител - ако е за запознаване и е до структура - разбиваме
						mailList.add(referent);
						if (nb.getNotificationCode() != null && nb.getNotificationCode().equals(BabhConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_DOC_ACCESS) ) {
							
							List<SystemClassif> children = nb.getSd().getChildrenOnNextLevel(BabhConstants.CODE_CLASSIF_ADMIN_STR , referent, new Date(), BabhConstants.CODE_DEFAULT_LANG);
							if (children != null) {
...
//			 kgMainMessages.send("тест2","-1");
	...
	public void saveMainMessage(UserNotifications msg, Set<Integer> users, Integer registraturaId, SystemData sd) {
		...
		if (msg.getMessageType() != null) {
				...
				if (msg.getMessageType().intValue() == - BabhConstants.CODE_ZNACHENIE_NOTIFF_EVENTS_DOC_ACCESS) {
		...
		try {
			...
			for (Integer userId : users) {
					...
					if (empl != null) {
						email = SearchUtils.asString( empl.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_CONTACT_EMAIL]);
					}
					
					SystemClassif user = sd.decodeItemLite(Constants.CODE_CLASSIF_USERS, userId, SysConstants.CODE_DEFAULT_LANG, new Date(), true);
					if (user != null) {
						Integer dubl = SearchUtils.asInteger( user.getSpecifics()[BabhClassifAdapter.USERS_INDEX_DUBL_MAIL]);
						if (dubl != null && email != null && dubl.equals(BabhConstants.CODE_ZNACHENIE_DA)) {
							curMsg.setEmailTo(email);
							curMsg.setSentToEmail(BabhConstants.CODE_ZNACHENIE_STATUS_MAIL_NOT_SEND);
						}
					}
	...
	public void saveEmailMessage(UserNotifications msg, Set<Integer> slujit, SystemData sd) {
		...
		try {
			...
			for (Integer slujitId : slujit) {
					...
					String email = SearchUtils.asString( empl.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_CONTACT_EMAIL]);
					if (email == null) {
						continue;
					}
