    	...
    	try {
			...
			for(Element el:model.getElements()) {
				...
				if(checkIf(etap)) {
					...
					//ptri da
					createConnectionData(etap.getNextOk(),  el ,1, 0, "Следващ");
					
					
					//pri ne
					createConnectionData(etap.getNextNot(),  el ,2, 3, "Алтернативен");
    ...
    private void createConnectionData(String next, Element el ,int fromPos, int toPos, String label) {
    	...
    	if(next!=null) {
			...
			for(int j=0; j<nextList.length;j++) {
				...
				if(nextEl!=null) {
					if(label!=null && label.equals("Алтернативен") && checkConect(el ,nextEl)) {
