...
//	private Integer periodAssign;//период на дата на поставяне
    ...
    /** Код на класификация "Тип документ" */
    public Integer getCodeVpisvane() {
    ...
    public Integer getCodeAdmStructura() {
        return BabhConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT;
    }
    
    /** Пощенска кутия" */
    public Integer getCodeMail() {
        return BabhConstants.CODE_ZNACHENIE_JOURNAL_MAILBOX;
    }
    
    /** Код на класификация "Изпълнение на процедура" */
    public Integer getCodeProcExe() {
        return BabhConstants.CODE_ZNACHENIE_JOURNAL_PROC_EXE;
    }

    
    /** Код на класификация "СЕОС/ССЕВ съобщение" */
    public Integer getCodeEgovMsg() {
        return BabhConstants.CODE_ZNACHENIE_JOURNAL_EGOVMESSAGE;
    }
	...
	void initData() {
		...
		obectList.add(new SelectItem(getCodeVpisvane(), "Вписване" ));
...
//		obectList.add(new SelectItem(getCodeEgovMsg(),getMessageResourceString(LABELS, "registratura.seosSsev")));
	...
	public void actionUnlock() {
		if (refType==BabhConstants.CODE_ZNACHENIE_JOURNAL_MAILBOX) {
			try {
				...
				JPA.getUtil().begin();
					...
					query.setParameter(OBJTYPE, BabhConstants.CODE_ZNACHENIE_JOURNAL_MAILBOX);
					query.executeUpdate();
					JPA.getUtil().commit();
...
//						objType = Long.parseLong(selectedRow.getObjectTip().toString());
						...
						Query query = JPA.getUtil().getEntityManager().createNativeQuery(qq);
						query.setParameter(OBJTYPE, refType).setParameter("obectId", obectId);
						query.executeUpdate();
					}
					JPA.getUtil().commit();
...
//	}
	...
	public void actionSearch(){
		...
		Query query;
		if (refType.equals(getCodeVpisvane())) {
			qq=" select v.id ,v.reg_nom_zaqvl_vpisvane , v.licenziant_type, lo.lock_date, lo.user_id, lo.lock_info, v.id_register  "
					+ " FROM LOCK_OBJECTS lo, vpisvane v  "
					+ " WHERE lo.object_id = v.id AND lo.OBJECT_TIP=:objType ";

			lockObjectsList.clear();
		} else if (refType==BabhConstants.CODE_ZNACHENIE_JOURNAL_DOC) {
			qq="select  "+DocDAO.formRnDocSelect("d.", JPA.getUtil().getDbVendorName())+" rnDoc,d.DOC_DATE dataDok,"+DialectConstructor.limitBigString(dialect, "d.OTNOSNO", 300)+"    otnosno , lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
		...
		} else 	if (refType==BabhConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT) {
			qq="select lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
					+ TIPOBJIDLOCKINF
					+ FROMLOCKOBJLO
					+ " WHERE  OBJECT_TIP=:objType ";
		} else	if (refType==BabhConstants.CODE_ZNACHENIE_JOURNAL_PROC_EXE) {
			qq="select  lo.LOCK_DATE dataZakl, lo.USER_ID userZakl, "
		...
		for (int i = 0; i < lockList.size(); i++) {
			...
			if (refType.intValue() == BabhConstants.CODE_ZNACHENIE_JOURNAL_VPISVANE) {
				tmp.setObjectId(Long.parseLong(op[0].toString()));
				tmp.setRnDoc((String)op[1]);
				...
				tmp.setLockInfo((String) op[5]);
			}
			if (refType==BabhConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT) {
			...
			/*if (refType==BabhConstants.CODE_ZNACHENIE_JOURNAL_EGOVMESSAGE) {
				tmp.setObjectId(Long.parseLong(op[3].toString()));
				tmp.setObjectTip(Long.parseLong(op[2].toString()));
				tmp.setUserId(Long.parseLong(op[1].toString()));
				if (op[0] instanceof java.util.Date) {
			...
			*/
			if (refType==BabhConstants.CODE_ZNACHENIE_JOURNAL_DOC) {
			...
			if (refType==BabhConstants.CODE_ZNACHENIE_JOURNAL_PROC_EXE) {
