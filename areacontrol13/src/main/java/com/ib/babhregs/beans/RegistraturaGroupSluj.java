...
import com.ib.babhregs.db.dao.RegistraturaGroupDAO;
	...
	public void initData() {
		...
		try {
				...
				if (getUserData(UserData.class).isDelovoditel()) {
					...
					String nameReg = getSystemData().decodeItem(BabhConstants.CODE_CLASSIF_REGISTRATURI, getUserData(UserData.class).getRegistratura(), getCurrentLang(), new Date());
...
//		this.specificsReferents  = Collections.singletonMap(BabhClassifAdapter.REFERENTS_INDEX_REGISTRATURA, this.idReg); 
	...
	public void actionSearch(){
		...
		try {
			...
			JPA.getUtil().runWithClose(() -> this.regGrSlujList = this.regGrDAO.findByRegistraturaId(this.usersRegList, Arrays.asList(BabhConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_EMPL), this.codeRef, this.name, this.info));
				...
				if(this.groupSluj.getReferentIds() != null && !this.groupSluj.getReferentIds().isEmpty()) {
					...
					for (Integer item : this.groupSluj.getReferentIds()) {
						...
						try {
							...
							scItem.setCodeClassif(BabhConstants.CODE_CLASSIF_ADMIN_STR);
							scItem.setCode(item);
							tekst = getSystemData().decodeItem(BabhConstants.CODE_CLASSIF_ADMIN_STR, item, getUserData().getCurrentLang(), new Date());		
	...
	public void actionSearch(Integer groupId) {
		...
		SelectMetadata smd = this.regGrDAO.findReferentsByIdGroup(groupId, BabhConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_EMPL, getUserData());
		...
		if (this.groupSluj.getReferentIds() != null) {
			...
			try {
				...
				for (Integer refCode : this.groupSluj.getReferentIds()) {
					...
					Integer refType = (Integer) getSystemData().getItemSpecific(BabhConstants.CODE_CLASSIF_ADMIN_STR, refCode, getCurrentLang(), new Date(), BabhClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE);
				
					if (refType.equals(Integer.valueOf(BabhConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO))) {
				...
				getSystemData().reloadClassif(BabhConstants.CODE_CLASSIF_GROUP_EMPL, false, false);
...
//		}
	...
	public void actionSave() {
		...
		try {
			...
			this.groupSluj.setGroupType(BabhConstants.CODE_ZNACHENIE_REG_GROUP_TYPE_EMPL);
