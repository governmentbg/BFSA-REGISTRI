...
import com.ib.babhregs.db.dao.RegistraturaMailBoxDAO;
import com.ib.babhregs.db.dto.RegistraturaMailBox;
import com.ib.babhregs.db.dto.RegistraturaMailBoxVar;
...
public class RegistraturaMailboxes extends IndexUIbean implements Serializable {	
	...
	public void actionChangeMailKey() {
		...
		if (this.mailKey != null) {
			...
			try {
				...
				String mailKeyTxt = getSystemData().decodeItem(BabhConstants.CODE_CLASSIF_VARIABLES_SETTINGS_MAIL_ACCOUNT, this.mailKey, getCurrentLang(), new Date());
	...
	public Integer returnMailKey(String mailKeyTxt) {
		...
		try {
			...
			List<SystemClassif> tmpSC = getSystemData().queryClassification(BabhConstants.CODE_CLASSIF_VARIABLES_SETTINGS_MAIL_ACCOUNT, mailKeyTxt, new Date(), getCurrentLang());
