...
public class RegistraturaData extends IndexUIbean  implements Serializable {	
			...
			if (JSFUtils.getRequestParameter(ID_OBJ) != null && !"".equals(JSFUtils.getRequestParameter(ID_OBJ))) {
				...
				if (idObj != null) {
					...
					JPA.getUtil().runWithClose(() -> {
						...
						this.filesList = new FilesDAO(getUserData()).selectByFileObject(this.registratura.getId(), BabhConstants.CODE_ZNACHENIE_JOURNAL_REISTRATURA);
					});
					
					if(Objects.equals(this.registratura.getValid(), BabhConstants.CODE_ZNACHENIE_DA)) {
						this.activeReg = true;
	...
	public void uploadFileListener(FileUploadEvent event){		
		...
		try {
			...
			JPA.getUtil().runInTransaction(() -> {
				new FilesDAO(getUserData()).saveFileObject(fileObject, this.registratura.getId(), BabhConstants.CODE_ZNACHENIE_JOURNAL_REISTRATURA); 
	...
	public void actionCheckEmail() {
		...
		if (!SearchUtils.isEmpty(this.registratura.getContactEmail()) && !ValidationUtils.isEmailValid(this.registratura.getContactEmail())) {
			
			JSFUtils.addMessage(FORM_TABS_REGISTRATURI + ":contactEmail", FacesMessage.SEVERITY_ERROR, String.format(getMessageResourceString(beanMessages, "dvijenie.emailNevaliden"), this.registratura.getContactEmail().trim()));			
