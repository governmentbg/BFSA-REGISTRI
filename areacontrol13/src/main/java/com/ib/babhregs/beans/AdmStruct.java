...
public class AdmStruct extends IndexUIbean {
	...
	public void actionSaveZveno() {
		...
		try {
			...
			referent.setRefType(BabhConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO);
	...
	public void actionSaveSluj() {
		...
		try {
			...
			referentSluj.setRefType(BabhConstants.CODE_ZNACHENIE_REF_TYPE_EMPL);
			...
			if(codeForRegistr!=null && !codeForRegistr.equals(0)) { 
				...
				try {
						...
						if(selectedZvenoClassif!=null) {
							referent.setRefRegistratura( (Integer) selectedZvenoClassif.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA]);
						}
					} catch (DbErrorException e) {
					LOGGER.error(e.getMessage(), e);
					JSFUtils.addGlobalMessage(FacesMessage.SEVERITY_ERROR, e.getMessage());	
	...
	public void selectForEdit(Object[] item) {
		viewTab = true;
		if(SearchUtils.asInteger(item[2]).equals(BabhConstants.CODE_ZNACHENIE_REF_TYPE_EMPL)) {
...
//		} 
		...
		if( referent.getExternalCode()==null || "".equals(referent.getExternalCode())) {
			JSFUtils.addMessage("admStructForm:tabsEditZvenaSluj:exCode", FacesMessage.SEVERITY_ERROR,getMessageResourceString(UI_beanMessages, MSGPLSINS, "Външен код за интеграция. Ако няма такъв, въведете -1 (минус едно)"));
			save = true;
		}
		return save;
	}
...
//				Integer zvenoType = (Integer) selectedZvenoClassif.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE];
//				if(zvenoType != null && zvenoType.intValue() == BabhConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO) {
					...
					if(selectedZvenoClassif!=null && selectedZvenoClassif.getCodePrev()!=0) {
				    	...
				    	registrParent = (Integer) selectedZvenoClassif.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA];
...
//				}
	...
	public void searchHistoryZveno() {
		searchHistory(referent.getCode(), BabhConstants.CODE_ZNACHENIE_REF_TYPE_ZVENO);
		isZveno = true;
	}
	
	public void searchHistoryEmpl() {
		searchHistory(referentSluj.getCode(), BabhConstants.CODE_ZNACHENIE_REF_TYPE_EMPL);
		isZveno = false;
	}
	...
	private boolean checkForLock() {
		...
		try { 
			Object[] obj = daoL.check(ud.getUserId(), BabhConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT, BabhConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT);
			if (obj != null) {
				 res = false;
				 
				 String msg = getSystemData().decodeItem(BabhConstants.CODE_CLASSIF_ADMIN_STR, Integer.valueOf(obj[0].toString()), getUserData().getCurrentLang(), new Date())   
	...
	public void lockAdmStr() {	
		...
		try { 
			JPA.getUtil().runInTransaction(() -> {
				daoL.lock(ud.getUserId(), BabhConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT, BabhConstants.CODE_ZNACHENIE_MENU_ADM_STRUCT, null);
			});
		} catch (DbErrorException e) {
	...
	public void actionLoadLeft() {
		try {
			leftEmpl = getSystemData().getChildrenOnNextLevel(BabhConstants.CODE_CLASSIF_ADMIN_STR_REPORTS, -1000, decodeDate, getCurrentLang());
		} catch (DbErrorException e) {
	...
	public void actionSaveLeftEmpl() {
			...
			try {
				...
				SystemClassif classifEmpl = getSystemData().decodeItemLite(BabhConstants.CODE_CLASSIF_ADMIN_STR_REPORTS, selectedEmpl, getCurrentLang(), decodeDate, false);
