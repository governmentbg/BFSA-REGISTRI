...
import com.ib.babhregs.db.dao.DocVidSettingDAO;
...
public class DocVidSettings extends IndexUIbean implements Serializable {	
	...
	public void initData() {
		...
		try {	
			...
			specificsIn.put(BabhClassifAdapter.PROCEDURI_INDEX_REGISTRATURA, this.idReg); // специфика за процедури по ид на регистратура
			specificsIn.put(BabhClassifAdapter.PROCEDURI_INDEX_STATUS, BabhConstants.CODE_ZNACHENIE_PROC_DEF_STAT_ACTIVE); // специфика по статус активна за процедурите
			//specificsIn.put(BabhClassifAdapter.PROCEDURI_INDEX_DOC_TYPE, BabhConstants.CODE_ZNACHENIE_DOC_TYPE_IN); // специфика по тип документ - входящ
			
			this.proceduriListDefIn = ((SystemData) getSystemData()).queryClassification(BabhConstants.CODE_CLASSIF_PROCEDURI, null, new Date(), getCurrentLang(), specificsIn);
			
			Map<Integer, Object> specificsOwn = new HashMap<>();
			specificsOwn.put(BabhClassifAdapter.PROCEDURI_INDEX_REGISTRATURA, this.idReg); // специфика за процедури по ид на регистратура
			specificsOwn.put(BabhClassifAdapter.PROCEDURI_INDEX_STATUS, BabhConstants.CODE_ZNACHENIE_PROC_DEF_STAT_ACTIVE); // специфика по статус активна за процедурите
			//specificsOwn.put(BabhClassifAdapter.PROCEDURI_INDEX_DOC_TYPE, BabhConstants.CODE_ZNACHENIE_DOC_TYPE_OWN); // специфика по тип документ - собствен
			
			this.proceduriListDefOwn = ((SystemData) getSystemData()).queryClassification(BabhConstants.CODE_CLASSIF_PROCEDURI, null, new Date(), getCurrentLang(), specificsOwn);
			
			this.docHarList = ((SystemData) getSystemData()).getSysClassification(BabhConstants.CODE_CLASSIF_CHARACTER_SPEC_DOC, new Date(), getCurrentLang());
	...
	public List<SelectItem> registriListByIdReg(Integer idRegistratura){
		...
		try {
			...
			String specific = (String) getSystemData().getItemSpecific(BabhConstants.CODE_CLASSIF_REGISTRATURI, idRegistratura, getCurrentLang(), new Date(), BabhClassifAdapter.REGISTRATURI_INDEX_REGISTRI);
			...
			for (String r : registersIds) {
				...
				Integer active = (Integer) getSystemData().getItemSpecific(BabhConstants.CODE_CLASSIF_REGISTRI, Integer.valueOf(r), getCurrentLang(), new Date(), BabhClassifAdapter.REGISTRI_INDEX_VALID);
				
				if (active.intValue() == BabhConstants.CODE_ZNACHENIE_DA) {
					String name = getSystemData().decodeItem(BabhConstants.CODE_CLASSIF_REGISTRI, Integer.valueOf(r), getCurrentLang(), new Date());
		...
		try {
			...
			if (idObj != null) {
				...
				JPA.getUtil().runWithClose(() -> {
					...
					this.filesList = new FilesDAO(getUserData()).selectByFileObject(this.dvSetting.getId(), BabhConstants.CODE_ZNACHENIE_JOURNAL_DOC_VID_SETT);
				});

				if (Objects.equals(this.dvSetting.getCreateDelo(), BabhConstants.CODE_ZNACHENIE_DA)) {
	...
	public boolean checkData() {
		...
		if(this.dvSetting.getBegNomer() == null && (this.registerAlg != null && this.registerAlg.equals(BabhConstants.CODE_ZNACHENIE_ALG_VID_DOC))) { 
		...
		if(this.dvSetting.getActNomer() == null && (this.registerAlg != null && this.registerAlg.equals(BabhConstants.CODE_ZNACHENIE_ALG_VID_DOC))) {
		...
		if(this.dvSetting.getStep() == null && (this.registerAlg != null && this.registerAlg.equals(BabhConstants.CODE_ZNACHENIE_ALG_VID_DOC))) {
	...
	public void actionSave() {
		...
		try {
			...
			if (this.createDelo) {
				this.dvSetting.setCreateDelo(BabhConstants.CODE_ZNACHENIE_DA);								
			} else {
				this.dvSetting.setCreateDelo(BabhConstants.CODE_ZNACHENIE_NE);	
			}
	...
	public void actionChangeRegister() {
		...
		try {
			...
			if (this.dvSetting.getRegisterId() != null) {
				...
				this.registerAlg = (Integer) getSystemData().getItemSpecific(BabhConstants.CODE_CLASSIF_REGISTRI, this.dvSetting.getRegisterId(), getCurrentLang(), new Date(), BabhClassifAdapter.REGISTRI_INDEX_ALG);
				
				if (this.registerAlg.equals(BabhConstants.CODE_ZNACHENIE_ALG_VID_DOC)) {
					if(!this.fromEdit) {
				...
				Integer registerTipDoc = (Integer) getSystemData().getItemSpecific(BabhConstants.CODE_CLASSIF_REGISTRI, this.dvSetting.getRegisterId(), getCurrentLang(), new Date(), BabhClassifAdapter.REGISTRI_INDEX_DOC_TYPE);
				...
				if (registerTipDoc != null) {						
					...
					if (Objects.equals(registerTipDoc, BabhConstants.CODE_ZNACHENIE_DOC_TYPE_IN)) {
						this.viewProcDefIn = true;					
					
					} else if (Objects.equals(registerTipDoc, BabhConstants.CODE_ZNACHENIE_DOC_TYPE_OWN)) {
						this.viewProcDefOwn = true;
						
					} else if (Objects.equals(registerTipDoc, BabhConstants.CODE_ZNACHENIE_DOC_TYPE_WRK)) {
						this.viewProcDefWork = true;					
					}
	...
	public void actionChangeVidDoc() {
		...
		if (this.dvSetting.getDocVid() != null) {
			...
			try {
				...
				SystemClassif item = getSystemData().decodeItemLite(BabhConstants.CODE_CLASSIF_DOC_VID_SETTINGS, code, getCurrentLang(), new Date(), false);
				if (item != null) {
