...
public class TestSystemData {
	...
	public static void setUp() throws Exception {
		sd = new SystemData();
		sd.getSysClassification(BabhConstants.CODE_CLASSIF_REGISTRI, null, 1);
	}
...
//	@Test
	public void testClassifAdmStruct() { // CODE_CLASSIF_ADMIN_STR само административната структура ЙЕРАРХИЧНА
		try {
			...
			LOGGER.info("\tREF_TYPE=" + item.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_REF_TYPE]);
			LOGGER.info("\tREGISTRATURA=" + item.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_REGISTRATURA]);
			LOGGER.info("\tCONTACT_EMAIL=" + item.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_CONTACT_EMAIL]);
...
//	@Test
	public void testClassifGroupEmployees() {
		...
		try {
			...
			List<SystemClassif> allGroups = sd.getSysClassification(BabhConstants.CODE_CLASSIF_GROUP_EMPL, date, SysConstants.CODE_DEFAULT_LANG);
			...
			Map<Integer, Object> specRegistratura = Collections.singletonMap(BabhClassifAdapter.REG_GROUP_INDEX_REGISTRATURA, registratura);
			List<SystemClassif> groupsByRegistratura = sd.queryClassification(BabhConstants.CODE_CLASSIF_GROUP_EMPL, null, date, SysConstants.CODE_DEFAULT_LANG, specRegistratura);
			...
			if (!groupsByRegistratura.isEmpty()) {
				...
				String emplCodes = (String) sd.getItemSpecific(BabhConstants.CODE_CLASSIF_GROUP_EMPL, selectedGroup, SysConstants.CODE_DEFAULT_LANG, date, BabhClassifAdapter.REG_GROUP_INDEX_MEMBERS);
				if (emplCodes != null) { // защото може и да е празна
					...
					for (String code : codes) {
						...
						LOGGER.info("\tEMAIL={}", empl.getSpecifics()[BabhClassifAdapter.ADM_STRUCT_INDEX_CONTACT_EMAIL]);
...
//	@Test
	public void testClassifRegistri() {
		try {
			...
			Map<Integer, Object> map = Collections.singletonMap(BabhClassifAdapter.REGISTRI_INDEX_DOC_TYPE, Optional.of(1));

			long t1 = System.currentTimeMillis();
			for (int i = 0; i < 100000; i++) {

				list = sd.queryClassification(BabhConstants.CODE_CLASSIF_REGISTRI, "индекс", null, 1, map);
			}
