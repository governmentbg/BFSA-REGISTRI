...
public class EdroMigrate {
	
	public static int seqVal = 6000000;
	...
	private static final Logger LOGGER = LoggerFactory.getLogger(EdroMigrate.class);
	...
	@SuppressWarnings("unchecked")	
	public static void transferEdro() {
		...
		String sqlMain = "SELECT nom, razreshenie, firm, addr, upr, dop_info, active, ime, notes, grupi, grupi_codes FROM temp_edro order by nom";
		...
		for (Object[] row : allDrebno) {
			...
			try {
				...
				String certS = SearchUtils.asString(row[1]);
				String firmAddr = SearchUtils.asString(row[2]);
				String addr = SearchUtils.asString(row[3]);
				String upr = SearchUtils.asString(row[4]);
				String zapoved = SearchUtils.asString(row[5]);
				String active = SearchUtils.asString(row[6]);
				String firmIme = SearchUtils.asString(row[7]);
				String notes = SearchUtils.asString(row[8]);
				String grupi = SearchUtils.asString(row[9]);
				String grupiCodes = SearchUtils.asString(row[10]);
				...
				ArrayList<EventDeinostVlpPredmet> predmeti = new ArrayList<EventDeinostVlpPredmet>();
				if (grupiCodes != null) {
					String[] codes = grupiCodes.split("\\|");
					for (String code : codes) {
						if (! code.trim().isEmpty()) {
							EventDeinostVlpPredmet edvp = new EventDeinostVlpPredmet();
							edvp.setId(getSeq("seq_event_deinost_vlp_predmet"));
							edvp.setPredmet(Integer.parseInt(code.trim()));
							edvp.setCodeClassif(564);
							edvp.setIdEventDeinostVlp(idEvent);
							predmeti.add(edvp);
						}
					}
...
//				ra.setPostCode(SearchUtils.asString(tek[4]));
				...
				if (! SearchUtils.isEmpty(certNom)) {
					...
					//cntSkipped ++;
					LOGGER.info("Няма номер на сертификат за едро с nom " + nom );
					//continue;
					certNom = "N/A";
				...
				String anot = "Заявление за издаване на разрешение за търговия на едро с ВЛП на " + firmIme ;
					
				doc.setOtnosno(anot);
				doc.setIndPlateno(BabhConstants.CODE_ZNACHENIE_DA);
				doc.setImeAdnUsluga("Издаване на разрешение за търговия на едро с ВЛП");
				doc.setCodeAdmUsluga("705");
				doc.setRegisterId(23);  
				...
				//CODE_ZNACHENIE_REG_OWN_PROIZVOLN_NOMER 
				...
				docU.setImeAdnUsluga("Издаване на разрешение за търговия на едро с ВЛП");
				docU.setCodeAdmUsluga("705");
				docU.setRegisterId(23);  
				...
				if (izmDat != null & izmNom != null) {
					...
					docIzm.setImeAdnUsluga("Издаване на разрешение за търговия на едро с ВЛП");
					docIzm.setCodeAdmUsluga("705");
					docIzm.setRegisterId(23);  
				...
				if (zapoved != null && ! SearchUtils.isEmpty(zapoved)) {
					
					vsZ = new VpisvaneStatus();
					vsZ.setId(getSeq("seq_vpisvane_status"));
					vsZ.setReasonDop(zapoved);
					...
					String zapovedUpper = zapoved.toUpperCase().replace("прекратен със Заповед № ".toUpperCase(), "").replace("ОТНЕТ СЪС ЗАПОВЕД №".toUpperCase(), "").replace("прекратен със Заповед ".toUpperCase(), "");
					
					
					
					String[] partsZ = zapovedUpper.split("/");
					
					if (partsZ.length == 1) {
						partsZ = zapovedUpper.split("ОТ");
					}
				...
				if (docIzm != null) {
					InvitroMigrate.insertDoc(docIzm);
				}
				...
				insertEventVlp(idVpisvane, idEvent, grupi);	
				insertEventVlpVid(idEvent, BabhConstants.CODE_ZNACHENIE_DEIN_VLP_TR_EDRO);
				
				for (EventDeinostVlpPredmet predmet : predmeti) {
					InvitroMigrate.insertEventVlpPredmet(predmet);
				}
		...
		LOGGER.info("Общо Обработени Едро : " + cnt );
		LOGGER.info( "Прoпуснати Едро : " + cntSkipped );
		LOGGER.info("Грешки : " + cntErrors);
		LOGGER.info(dat + " - " + new Date());
		
		
		System.out.println("Общо Обработени Едро : " + cnt);
		System.out.println("Прoпуснати Едро : " + cntSkipped);
	...
	private static void insertEventVlp(Integer idVpis, Integer idEvent, String dopInfo) throws DbErrorException {
		String insertSql = "INSERT INTO event_deinost_vlp (id, id_vpisvane, user_reg, date_reg, dop_info) VALUES (?,?,?,?,?);";
		...
		try {
			...
			query.setParameter(5, new TypedParameterValue(StandardBasicTypes.STRING,dopInfo));
