...
import bg.bulsi.bfsa.service.ApplicationS2274Service;
...
@Slf4j
@RestController
@RequestMapping("/s2274-applications")
@SecurityRequirement(name = "bfsaapi")
@RequiredArgsConstructor(access = AccessLevel.PACKAGE)
public class ApplicationS2274Controller {

    private final ApplicationS2274Service service;
    ...
    public ResponseEntity<?> register(@RequestBody final ApplicationS2274DTO dto,
                                      @RequestHeader(HttpHeaders.ACCEPT_LANGUAGE) final Locale locale) {
        Language language = LocaleUtil.getLanguage(locale, languageService);
    ...
    public ResponseEntity<?> approve(@PathVariable final Long recordId,
        ...
        return ResponseEntity.ok(BaseApplicationDTO.builder()
                .recordId(record.getId())
                .serviceType(record.getServiceType())
                .regNumber(record.getContractorPaper().getRegNumber())
