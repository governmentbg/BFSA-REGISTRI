...
import bg.bulsi.bfsa.service.ApplicationS2711Service;
...
@Slf4j
@RestController
@RequestMapping("/s2711-applications")
@SecurityRequirement(name = "bfsaapi")
@RequiredArgsConstructor(access = AccessLevel.PACKAGE)
public class ApplicationS2711Controller {

    private final ApplicationS2711Service service;
    ...
    public ResponseEntity<?> register(@RequestBody final ApplicationS2711DTO dto,
                                      @RequestHeader(HttpHeaders.ACCEPT_LANGUAGE) final Locale locale) {
        Language language = LocaleUtil.getLanguage(locale, languageService);
    ...
    public ResponseEntity<?> forCorrection(@PathVariable final Long recordId,
                                           ...
                                           @RequestHeader(HttpHeaders.ACCEPT_LANGUAGE) final Locale locale) {
        if (dto.getId() == null || !recordId.equals(dto.getId())) {
