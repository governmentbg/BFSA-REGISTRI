...
import bg.bulsi.bfsa.service.ApplicationS1366Service;
...
@Slf4j
@RestController
@RequestMapping("/s1366-applications")
@SecurityRequirement(name = "bfsaapi")
@RequiredArgsConstructor(access = AccessLevel.PACKAGE)
public class ApplicationS1366Controller {

    private final ApplicationS1366Service service;
    ...
    public ResponseEntity<?> register(@RequestBody final ApplicationS1366DTO dto,
                                      @RequestHeader(HttpHeaders.ACCEPT_LANGUAGE) final Locale locale) {
        Language language = LocaleUtil.getLanguage(locale, languageService);
    ...
    public ResponseEntity<?> approve(@PathVariable final Long recordId,
                                     @RequestBody final List<ApproveFoodTypesDTO> approvedFoodTypes,
                                     @RequestHeader(HttpHeaders.ACCEPT_LANGUAGE) final Locale locale) {
        Language language = LocaleUtil.getLanguage(locale, languageService);
        Record record = service.approve(recordId, approvedFoodTypes, language);
