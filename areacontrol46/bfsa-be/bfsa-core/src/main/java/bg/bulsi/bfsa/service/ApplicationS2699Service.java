...
import bg.bulsi.bfsa.repository.ApplicationS2699Repository;
...
public class ApplicationS2699Service {

    private final ApplicationS2699Repository repository;
    ...
    private final ServiceType SERVICE_TYPE = ServiceType.S2699;
    public static final String REGISTER_CODE = Constants.CLASSIFIER_REGISTER_0002038_CODE;
    public static final String DIRECTORATE_CODE = Constants.PPP_DIRECTORATE_CODE;

    @Transactional(readOnly = true)
    public ApplicationS2699DTO findByRecordId(final Long recordId, final Language language) {
        return ApplicationS2699DTO.ofRecord(recordService.findById(recordId), language);
    }

    @Transactional(readOnly = true)
    public List<ApplicationS2699DTO> findAllByApplicantIdAndStatus(final Long applicantId, final ApplicationStatus status, final Language language) {
        return ApplicationS2699DTO.of(repository.findAllByRecord_Applicant_IdAndStatus(applicantId, status), language);
    }

    @Transactional
    public Record register(final ApplicationS2699DTO dto, final ServiceRequestWrapper wrapper, final Language language) {
        ...
        ApplicationS2699 application = ApplicationS2699.builder()
                .serviceRequestWrapper(wrapper)
                .status(ApplicationStatus.ENTERED).build();

        // TODO Add AddressType fumigation address type
        application.setSeedTreatmentFacilityAddress(Address.builder()
                .phone(dto.getSeedTreatmentFacilityAddress().getPhone())
                .mail(dto.getSeedTreatmentFacilityAddress().getMail())
                .url(dto.getSeedTreatmentFacilityAddress().getUrl())
                .settlement(settlementService.findByCode(dto.getSeedTreatmentFacilityAddress().getSettlementCode()))
                .fullAddress(settlementService.getInfo(dto.getSeedTreatmentFacilityAddress().getSettlementCode(), language.getLanguageId()))
                .address(dto.getSeedTreatmentFacilityAddress().getAddress())
                .fullAddress(dto.getSeedTreatmentFacilityAddress().getFullAddress())
                .build()
        );

        Contractor treatmentSeedControlPerson = baseApplicationService.buildContractor(dto.getCh83CertifiedPerson(), errors);
        baseApplicationService.hasPaperCheck(treatmentSeedControlPerson, errors, ServiceType.S2701);
        application.setCh83CertifiedPerson(treatmentSeedControlPerson);
        ...
        record.setApplicationS2699(application);
    ...
    public Record approve(final Long recordId, final ApproveDTO dto) {
        ...
        ApplicationS2699 application = record.getApplicationS2699();
        if (application == null) {
    ...
    public Record refuse(final Long recordId) {
        Record record = recordService.findById(recordId);
        record.getApplicationS2699().setStatus(ApplicationStatus.REJECTED);
    ...
    public Record forCorrection(final Long recordId, String message, final Language language) {
        Record record = recordService.findById(recordId);
        record.getApplicationS2699().setStatus(ApplicationStatus.FOR_CORRECTION);
